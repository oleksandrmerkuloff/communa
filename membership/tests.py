...
class MembershipAPITest(APITestCase):
    ...
    def test_retrieve_member(self):
        ...
        response = self.client.get(reverse("membership-detail", args=[member.id]))

        self.assertEqual(response.status_code, status.HTTP_200_OK)

    def test_member_not_found(self):
        response = self.client.get(reverse("membership-detail", args=[uuid4()]))
    ...
    def test_head_can_update_member(self):
        ...
        response = self.client.patch(
            reverse("membership-detail", args=[member.id]), {"apartment_number": 15}
        )
    ...
    def test_unauthorized_update(self):
        ...
        response = self.client.patch(
            reverse("membership-detail", args=[member.id]), {"apartment_number": 100}
        )
    ...
    def test_head_can_delete_member(self):
        ...
        response = self.client.delete(reverse("membership-detail", args=[member.id]))

        self.assertEqual(response.status_code, status.HTTP_204_NO_CONTENT)
        self.assertFalse(Membership.objects.filter(id=member.id).exists())
    ...
    def test_created_member_has_resident_role(self):
        ...
        self.assertEqual(member.role, Membership.MemberRole.RESIDENT)

    def test_head_role_is_preserved(self):
        self.assertEqual(self.head_membership.role, Membership.MemberRole.HEAD)
